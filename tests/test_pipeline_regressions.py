...
class TestRequirementGrounding:
    """Extracted requirements must be supported by the source document.

    A run against the wilderness weather station specification produced
    requirements for humidity readings and light intensity control. Neither
    word occurs anywhere in the document, which covers temperature, pressure,
    sunshine, rainfall and wind. Both fabrications carried full acceptance
    criteria, deliverables and test cases downstream.

    The agent prompt already forbids this and the extractor runs at temperature
    0.0, so the instruction is not the lever. The document is, and it can be
    checked mechanically.
    """

    HALLUCINATED = [
        "Record and store humidity readings with humidity_sensor and "
        "humidity_value in percentage",
        "Remotely adjust and monitor light intensity using a light sensor",
    ]

    GROUNDED = [
        "Collect temperature readings every minute",
        "Calculate and store pressure averages periodically",
        "Collect sunshine duration data periodically",
        "Collect wind speed and direction readings",
        "Record rainfall volume data",
        "Store weather data persistently with timestamps",
        "Report faults in instruments, power and communication hardware to "
        "the management system",
        "Manage power so batteries charge when conditions permit",
        "Update the embedded software remotely by satellite",
        "Remotely control a weather station in the event of system problems",
        "Transmit aggregated data to the data management system on request",
        "Monitor the health of the station and report problems to the "
        "maintenance system",
    ]

    @pytest.fixture(scope="class")
    def source_text(self):
        """The real specification the hallucinations were produced from."""
        from docx import Document

        path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                            "src", "autoAgile", "tests", "docs",
                            "Wilderness_Weather_Station.docx")
        return "\n".join(p.text for p in Document(path).paragraphs)

    def test_the_document_really_lacks_the_invented_subjects(self, source_text):
        """Pins the premise. If a future document mentions humidity, the other
        tests in this class would be asserting the wrong thing."""
        lowered = source_text.lower()

        assert "humidity" not in lowered
        assert "light" not in lowered
        assert "temperature" in lowered and "pressure" in lowered

    @pytest.mark.parametrize("description", HALLUCINATED)
    def test_invented_requirements_are_rejected(self, description, source_text):
        from agents.grounding import check_requirement

        outcome = check_requirement({"description": description}, source_text)

        assert outcome["grounded"] is False, outcome

    @pytest.mark.parametrize("description", GROUNDED)
    def test_genuine_requirements_are_kept(self, description, source_text):
        """Wrongly discarding a real requirement is the failure mode already
        fixed twice here; it must not be reintroduced."""
        from agents.grounding import check_requirement

        outcome = check_requirement({"description": description}, source_text)

        assert outcome["grounded"] is True, outcome

    def test_paraphrase_does_not_cause_a_rejection(self, source_text):
        """The document says "barometric pressure"; a requirement saying
        "pressure sensor" must still ground."""
        from agents.grounding import check_requirement

        outcome = check_requirement(
            {"description": "Sample the barometric pressure sensor"}, source_text)

        assert outcome["grounded"] is True, outcome

    def test_rejected_requirements_are_reported_not_dropped_silently(self, source_text):
        from agents.grounding import filter_grounded

        requirements = (
            [{"id": f"H-{i}", "description": d}
             for i, d in enumerate(self.HALLUCINATED)]
            + [{"id": f"G-{i}", "description": d}
               for i, d in enumerate(self.GROUNDED)]
        )

        kept, rejected = filter_grounded(requirements, source_text)

        assert len(kept) == len(self.GROUNDED)
        assert len(rejected) == len(self.HALLUCINATED)
        assert all(r["missing_terms"] for r in rejected), \
            "a rejection must say which vocabulary was absent"
        assert all("coverage" in r for r in rejected)

    def test_grounding_is_skipped_when_there_is_no_source(self):
        """No document means nothing to check against; keep everything."""
        from agents.grounding import filter_grounded

        requirements = [{"id": "REQ-001", "description": "Record humidity"}]

        kept, rejected = filter_grounded(requirements, "")

        assert kept == requirements
        assert rejected == []

    def test_threshold_is_configurable(self, monkeypatch, source_text):
        """Tuned on one document, so a deployment must be able to loosen it."""
        from agents.grounding import check_requirement

        monkeypatch.setenv("REQUIREMENT_GROUNDING_THRESHOLD", "0.0")
        outcome = check_requirement({"description": self.HALLUCINATED[0]}, source_text)
